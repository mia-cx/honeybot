...
export class DuplicateDetector {
  ...
  record(message: Message<true>, config: GuildConfig): DuplicateDetectionResult {
    if (!config.crosschannelEnabled) return { matched: false, channelIds: [] };

    const normalized = normalizeText(message.content);
    const domainKey = domains(message.content).join(',');
    const attachmentKey = message.attachments.map((attachment) => attachment.name ?? attachment.id).join(',');
    const signal = normalized || domainKey || attachmentKey;
    if (!signal) return { matched: false, channelIds: [] };

    const now = Date.now();
    const key = `${message.guildId}:${message.author.id}:${signal}`;
    const windowMs = config.crosschannelWindowSeconds * 1000;
    const fresh = (this.entries.get(key) ?? []).filter((entry) => now - entry.timestamp <= windowMs);

    fresh.push({ userId: message.author.id, channelId: message.channelId, messageId: message.id, timestamp: now });
    ...
    return { matched: channelIds.length >= config.crosschannelChannelThreshold, channelIds };
  }
